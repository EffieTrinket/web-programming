...
$complaints = $registerObj->filterStudentComplaints($user_id, $category, $department_id, $complaint_id);
...
<head>
    <meta charset="UTF-8">
    <title>Student Dashboard | Crimson</title>
    <link rel="stylesheet" href="../Style/student_des.css">
</head>
<body>
    <div class="dashboard-container">
        <aside class="sidebar">
            <h2>Crimson</h2>
            <ul>
                <li><a href="#" class="active">My Complaints</a></li>
                <hr>
                <li><a href="file_complaint.php">File a Complaint</a></li>
                <hr>
                <li><a href="../login&regis/logout.php" class= "logout" onclick="return confirm('Are you sure you want to log out?')">Log out</a> </li>
...
<br><br>
                    ...
                    <a href="student_dashboard.php" class="clear-btn">Clear</a>
                </form>
            </section>
                        ...
                        <?php if (!empty($complaints)): ?>
                            <?php foreach ($complaints as $index => $complaint): ?>
                                <tr>
                                    ...
                                    <td class="status"><?= htmlspecialchars($complaint['status'] ?? 'Pending') ?></td>
                                    <td><?= htmlspecialchars($complaint['date_filed']) ?></td>
                                    <td><a href="view_complaint.php?id=<?= urlencode($complaint['complaint_id']) ?>" class="view-btn">View</a></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
